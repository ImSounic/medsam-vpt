...
\begin{equation}
  ...
  \label{eq:drift}
\end{equation}
where $U(x,b) \in \mathbb{R}^{65536 \times 32}$ is the decoder's upscaled mask embedding for that image and prompt, with the $256 \times 256$ positions as samples and the 32 channels as features. It costs one extra base decoder forward per image, plus a base encoder forward when the encoder was adapted. A failure is a prediction with Dice below 0.5; both signals are scored by AUROC and AUPRC (high drift or low $\hat{u}$ positive), and $\hat{u}$ is compared with the true IoU by a ten-bin expected calibration error.
