...
def per_image_csv_for_seed(
    seed: int, training: str, method: str, dataset: str, perturb: int
) -> Path:
    ...
    if seed == 0:
        ...
        ckpt_dir = (
            REPO_ROOT / "bbox_robustness" / f"results_seed{seed}{suffix}" / "per_image"
        )
...
def load_per_seed_means(
    method: str, training: str, dataset: str, perturb: int
) -> list[float]:
...
def run_paired_t_seeds(
    diffs: list[float], greater_is_better: bool
) -> tuple[float, float]:
...
def run_paired_wilcoxon(
    a: pd.Series, b: pd.Series, greater_is_better: bool
    ...
    if len(diff) < 5 or (diff == 0).all():
        return (
            ...
            float("nan"),
            float("nan"),
            float(diff.median()) if len(diff) else float("nan"),
        )
    alt = "greater" if greater_is_better else "less"
    try:
...
def evaluate_claim(claim: dict) -> ClaimResult:
    ...
    if claim.get("invert_for_pass", False):
        ...
        seed_t, seed_p = run_paired_t_seeds(
            diffs, greater_is_better=claim["greater_is_better"]
        )
        pass_seed = (seed_p < 0.05) if not np.isnan(seed_p) else None
    ...
    if all_a:
        ...
        if claim.get("invert_for_pass", False):
            n_img, w_stat, img_p, med = run_paired_wilcoxon(
                a_pool, b_pool, greater_is_better=True
            )
            pass_image = (img_p >= 0.05) if not np.isnan(img_p) else None
        else:
            n_img, w_stat, img_p, med = run_paired_wilcoxon(
                a_pool, b_pool, greater_is_better=claim["greater_is_better"]
            )
            pass_image = (img_p < 0.05) if not np.isnan(img_p) else None
    else:
...
def write_markdown(results: list[ClaimResult], out_path: Path) -> None:
    ...
    for r in results:
        ...
        seed_t = (
            f"{r.seed_t:+.3f}"
            if r.seed_t is not None and not np.isnan(r.seed_t)
            else "-"
        )
        seed_p = (
            f"{r.seed_p_one_sided:.4f}"
            if r.seed_p_one_sided is not None and not np.isnan(r.seed_p_one_sided)
            else "-"
        )
        ...
        if r.image_n is not None:
            ...
            img_w = (
                f"{r.image_w:.0f}"
                if r.image_w is not None and not np.isnan(r.image_w)
                else "-"
            )
            img_p = (
                f"{r.image_p_one_sided:.2e}"
                if r.image_p_one_sided is not None and not np.isnan(r.image_p_one_sided)
                else "-"
            )
            img_md = (
                f"{r.image_median_diff:+.4f}"
                if r.image_median_diff is not None
                else "-"
            )
            img_ok = (
                "PASS" if r.pass_image else ("FAIL" if r.pass_image is False else "-")
            )
        else:
            img_n = img_w = img_p = img_md = img_ok = "-"
        lines.append(
            f"| **{r.name}** | {len(r.seed_diffs)} | {ms} | {seed_t} | {seed_p} | {seed_ok} | {img_n} | {img_w} | {img_p} | {img_md} | {img_ok} |"
        )
    lines.append("")
    ...
    lines.append("")
    lines.append(
        "- For C1a/C1b/C2: we expect both `seed pass` and `img pass` to be PASS."
    )
    lines.append(
        "- For C3: we expect both to be PASS, meaning the data does NOT support "
...
def main() -> int:
    ...
    for r in results:
        ...
        print(
            f"  {r.name}: n_diffs={len(r.seed_diffs)}, "
