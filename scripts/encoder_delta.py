...
for method in ['decoder_only', 'vpt_shallow', 'vpt_deep', 'lora', 'full_ft']:
    ...
    # Only compare keys present in both base and trained
    encoder_deltas = []
    for k, v in trained_state.items():
        if 'image_encoder' not in k:
            continue
        if k not in base_state:
            continue  # LoRA/VPT add new params not in base; handled separately
    ...
    if encoder_deltas:
        ...
        results[method] = 0.0  # decoder_only doesn't touch encoder; VPT/LoRA add new params (handled separately)
